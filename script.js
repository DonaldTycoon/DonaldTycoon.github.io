...
function Data(value) {
    ...
    fetch(fetchUrl).then(function(res){ return res.text()}).then(function(html) {
        ...
        toHTML(JSON.parse(doc.body.innerHTML.replace(/<\/?[^>]+>/gi, '').replace('|')));
    })
    .catch(function(err) {
...
function inventory(data, save) {
    ...
    console.log('data.inventory.' + item +'.amount');
    console.log(Object.keys('data.inventory.' + item +'.amount'));
    console.log(data.inventory.group_card|fisher|Fisherman.amount)
